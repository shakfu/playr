...
/// The cursor row: a background, or reversed where colour is off.
fn cursor_style(app: &Screen<'_>) -> Style {
    let p = app.palette();
...
fn draw_sampler(app: &Screen<'_>, f: &mut Frame, area: Rect) -> (u32, Option<sampler::Scale>) {
    ...
    let peaks = match sampler::peaks_of(app.sampler, current) {
        Ok(peaks) => peaks,
        Err(hint) => {
            let block = list_block(p, name.as_deref().unwrap_or(""));
    ...
    let colours = |c: usize| {
        if Some(c) == playhead {
            ...
            (p.accent, p.accent_peak)
        } else {
            (p.outside, p.outside_peak)
        }
    };
...
fn list_block<'a>(p: &Palette, title: &'a str) -> Block<'a> {
...
fn draw_tracks(
    ...
    let list = List::new(items)
        .block(list_block(p, title))
        .highlight_style(cursor_style(app));
...
fn draw_library(app: &Screen<'_>, f: &mut Frame, area: Rect) -> Scroll {
    ...
    let scroll = draw_tracks(app, f, area, &title, tracks, app.lists.library, |t| {
        selected.contains(t.path.as_str())
    });
...
fn draw_bar(app: &Screen<'_>, f: &mut Frame, area: Rect) {
    ...
    let prompt = match app.input {
        Input::SavePlaylist(name) => Some(Line::from(vec![
            Span::styled("save playlist as: ", Style::default().fg(p.accent)),
    ...
    if let Some(msg) = app.message {
        f.render_widget(
            Paragraph::new(Span::styled(msg, Style::default().fg(p.notice))),
            left,
        );
    } else if playing {
        let cells = (left.width as usize).saturating_sub(3);
        if cells > 0 {
            let bar = level_bar(p, app.snapshot.loudness, app.snapshot.peak, cells);
...
fn draw_marks(app: &Screen<'_>, f: &mut Frame, area: Rect) {
    ...
    f.render_widget(
        Paragraph::new(Span::styled(row, Style::default().fg(p.mark))),
        area,
    );
}
...
fn zone_colour(p: &Palette, i: usize, cells: usize) -> Color {
    match meter::zone(meter::level_at((i as f32 + 0.5) / cells as f32)) {
        meter::Zone::Red => p.red,
        meter::Zone::Yellow => p.yellow,
        meter::Zone::Green => p.green,
    }
}
...
fn level_bar(p: &Palette, loudness: Option<f32>, peak: Option<f32>, cells: usize) -> Line<'static> {
    ...
    spans.extend((0..cells).map(|i| {
        let zone = Style::default().fg(zone_colour(p, i, cells));
        match i {
...
fn level_readout(p: &Palette, loudness: Option<f32>, peak: Option<f32>) -> Vec<Span<'static>> {
    ...
    vec![
        Span::styled(format!("{lufs} LUFS  "), Style::default().fg(p.dim)),
        Span::styled(
            format!("pk {pk}  "),
            Style::default().fg(if clipping { p.clip } else { p.dim }),
        ),
    ]
}
