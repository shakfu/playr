...
- **Zoom.** `z` and `Z` zoom around the playhead, down to 64 frames a column; `0` shows the whole track. Columns start on 32-frame boundaries, so a hit never shows in the column before it.
...
1. **Reaction time.** `b` marks when the key is pressed, after the sound has passed. `:mark TIME` places a mark exactly; `,` and `.` seek to a mark to check it.
...
- **Hearing a region on its own.** A region cannot be looped or auditioned yet.
- **Marks away from the playhead in the sampler view.** Marks are placed at the playhead, or at a typed time.
